...
function createShareSheet() {
    ...
    sheetEl.innerHTML = `
        ...
        <div class="section-header">Share to</div>
        <div class="grid"></div>
        <div class="privacy-badge">Privacy First • Trackers Scrubbed</div>
    `;
    ...
    shareSheet = {
        ...
        toolsGrid: sheetEl.querySelector('.tools-grid'),
        close
    };
    return shareSheet;
}
...
function showShareSheet(title, url, text) {
    ...
    sheetObj.grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 20px; opacity: 0.5;">Loading Platforms...</div>';
    ...
    browser.runtime.sendMessage({ action: "getPlatforms" })
        .then(response => {
            if (!response || !response.platforms) {
                ...
                sheetObj.grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: red;">Error loading platforms.</div>';
                return;
            }

            const platforms = response.platforms;
            sheetObj.grid.innerHTML = '';
            sheetObj.toolsGrid.innerHTML = '';

            Object.keys(platforms).forEach(id => {
                if (id === 'share-custom' || id === 'share-screenshot' || id === 'share-screenshot-full') return;
                ...
                item.onclick = async (e) => {
                    ...
                    if (id === 'share-save' || id === 'share-print') {
        ...
        .catch(err => {
            ...
            sheetObj.grid.innerHTML = `
                <div style="grid-column: 1/-1; text-align: center; padding: 20px;">
                    <div style="color: #ff3b30; margin-bottom: 12px;">Connection Error</div>
                    <div style="font-size: 13px; opacity: 0.7; margin-bottom: 20px;">The background script is not responding. Try refreshing the page.</div>
                </div>
            `;
        });
}
...
browser.runtime.onMessage.addListener((message) => {
    if (message.action === "notifyThread") {
